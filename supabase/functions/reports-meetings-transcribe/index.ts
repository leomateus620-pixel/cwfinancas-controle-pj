...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (body.action === "start_session") {
      ...
      return new Response(JSON.stringify({ meeting_session_id: data.id, status: "recording" }), {
        headers: corsHeaders,
      });
    }
    ...
    await supabase.from("meeting_audit_logs").insert({
      user_id: user.id,
      entity_type: "meeting_session",
      entity_id: input.meeting_session_id,
      action: "meeting_finished",
      metadata: { transcript_length: transcript.length },
    });

    return new Response(
      JSON.stringify({ status: "finished", transcript_text: transcript, topic_summary: topicSummary }),
      { headers: corsHeaders },
    );
  } catch (e) {
