...
export function MeetingTranscriptPanel({ lines, status }: MeetingTranscriptPanelProps) {
  const dynamicHeight = Math.min(360, 120 + lines.length * 18);

  return (
    <div
      className="liquid-glass rounded-2xl p-4 transition-all duration-300 md:p-5"
    ...
    >
      <div className="mb-3 flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground">
          {status === "recording" ? "Capturando" : status === "paused" ? "Pausado" : "Aguardando"}
        </span>
      </div>
      ...
      <div className="max-h-[280px] overflow-y-auto rounded-xl border border-white/40 bg-white/50 p-3 text-sm leading-relaxed">
        {lines.length === 0 ? (
          <p className="text-muted-foreground">
            Ao iniciar a reunião, a transcrição começa automaticamente neste card.
          </p>
        ) : (
