...
export function MeetingTranscriptPanel({ lines, status }: Props) {
  ...
  return (
    <div className="liquid-glass rounded-2xl p-4 md:p-5 transition-all duration-300" style={{ minHeight: dynamicHeight }}>
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2 text-sm font-semibold">
          <ScrollText className="h-4 w-4 text-primary" />
          Transcrição em tempo real
        </div>
        <span className="text-xs text-muted-foreground">{status === "recording" ? "Capturando" : "Aguardando"}</span>
      </div>
      <div className="max-h-[280px] overflow-y-auto rounded-xl border border-white/40 bg-white/50 p-3 text-sm leading-relaxed">
        {lines.length === 0 ? (
          ...
          lines.map((line, i) => (
            <p key={`${line}-${i}`} className="mb-2 last:mb-0">• {line}</p>
          ))
        )}
      </div>
    </div>
